        ...
        if f != 0:
            if lower_limit_filter:
                if f >= lower_limit_val:
                    df.loc[co, 'Lower Limit Filter'] = 1

                else:
                    df.loc[co, 'Lower Limit Filter'] = 0
            else:
                df.loc[co, 'Lower Limit Filter'] = 0
        else:
            df.loc[co, 'Lower Limit Filter'] = 0
        ...
        if f != 0:
            if act_pow_filter:
                if df.loc[co, 'Active Power'] >= kw1 and df.loc[co, 'Active Power'] <= kw2 and df.loc[co, 'Pitch Angle'] >= (pit_a2 - pit_a1) / (((kw2 - kw1) * (df.loc[co, 'Active Power'] - kw1)) + kw1):
                    df.loc[co, 'Active Power Filter'] = 0
                    ...
                    df.loc[co, 'Active Power Filter'] = 1
...
def PlotData(df):
    ...
    df_filt = df[(df['Lower Limit Filter'] == 1) & (df['Active Power Filter'] == 1)]
    ...
    plt.plot(df_filt['Active Power'],df_filt['Pitch Angle'],'r.')
